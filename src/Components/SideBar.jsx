import React, { useEffect, useState } from 'react'
import { Link } from 'react-router-dom';
import { FaArrowRight } from 'react-icons/fa6';

const SideBar = () => {
    const [popularBlogs, setPopularBlogs] = useState([]);

    useEffect(() => {
        fetch('./blogsData.json').then(res => res.json()).then(data => setPopularBlogs(data.slice(0, 15)))
    }, [])

    return (
        <>
            <div>
                <div className="">
                    <h3 className="text-2xl font-semibold px-4">Latest Blogs</h3>
                    <div className="">
                        {
                            popularBlogs.slice(0, 5).map(blog => <div key={blog.id} className='my-5 border-b-2 border-spacing-2 px-4'>
                                <h4 className='font-medium mb-2'>{blog.title}</h4>
                                <Link to='/' className="inline-flex py-3 font-semibold hover:bg-white hover:text-red-500 transition-all duration-200 ease-in">Read More <FaArrowRight className='mt-1.5 ml-2' /></Link>
                            </div>)
                        }
                    </div>
                </div>
                {/* // popular blogs */}
                <div className="">
                    <h3 className="text-2xl font-semibold px-4 mt-20">Popular Blogs</h3>
                    <div className="">
                        {
                            popularBlogs.slice(5, 10).map(blog => <div key={blog.id} className='my-5 border-b-2 border-spacing-2 px-4'>
                                <h4 className='font-medium mb-2'>{blog.title}</h4>
                                <Link to='/' className="inline-flex py-3 font-semibold hover:bg-white hover:text-red-500 transition-all duration-200 ease-in">Read More <FaArrowRight className='mt-1.5 ml-2' /></Link>
                            </div>)
                        }
                    </div>
                </div>
            </div>
        </>
    )
}

export default SideBar
